...
class ARIMA(Environment):
    """
    Description: Simulates an autoregressive integrated moving-average time-series.
    """
    ...
    def step(self, u):
        ...
        if(self.noise_list is not None):
            self.x, self.delta_i_x, self.noise, self.x_new = self._step(self.x, self.delta_i_x, self.noise, self.noise_list[self.q + self.T - 1])
        else:
            if(self.noise_distribution == 'normal'):
                self.x,self.delta_i_x, self.noise, self.x_new = self._step(self.x, self.delta_i_x,self.noise, \
