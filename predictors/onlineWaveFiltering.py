...
class OnlineWaveFiltering(Predictor):
	...
	def update_parameters(self, y_true):
		...
		#gradient update
		self.M = self.M - 2 * self.lr * np.outer(y_diff.reshape(-1, 1), self.X_approx)

		#projection
		frob = np.linalg.norm(self.M, ord = 'fro')
		if(frob > self.R_m):
			self.M *= self.R_m / frob
